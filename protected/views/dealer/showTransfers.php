...
<table class="list">
    ...
    <?php foreach ($model as $transfer): ?>
        <tr>
            ...
            <td><?php echo((($transfer->sender->id != Yii::app()->user->id) && ($transfer->sender->parent->id != Yii::app()->user->id)) || ($transfer->receiver->parent->id != Yii::app()->user->id) ? '' : CHtml::link("", $this->createUrl('canceltransfer', array('id' => $transfer->id)), array('class' => 'cancel_transfer', 'title' => 'Отменить'))) ?></td>
        </tr>
    <?php endforeach; ?>
</table>
